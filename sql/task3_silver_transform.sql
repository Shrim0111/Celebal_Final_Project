...
WHERE rn = 1 AND title IS NOT NULL AND TRIM(title) <> ''AND author IS NOT NULL AND TRIM(author) <> ''AND price > 0 AND stock >= 0 AND published_on IS NOT NULL
  AND NOT EXISTS (SELECT 1 FROM silver_books sb WHERE sb.book_id = rb.book_id);
...
WHERE rn = 1 AND name IS NOT NULL AND TRIM(name) <> '' AND email IS NOT NULL AND email LIKE '%@%.%' AND joined_on IS NOT NULL AND membership IS NOT NULL AND TRIM(membership) <> ''
  AND NOT EXISTS (SELECT 1 FROM silver_customers sc WHERE sc.customer_id = rc.customer_id);
...
-- dedupe by order_id (keep latest ingested_at), join silver_books to get price for order_value
...
WHERE rn = 1 AND quantity > 0 AND order_date IS NOT NULL AND status IS NOT NULL AND TRIM(status) <> ''
  AND NOT EXISTS (SELECT 1 FROM silver_orders so WHERE so.order_id = o.order_id);
...
SELECT
    ...
    CASE
        WHEN NULLIF(TRIM(loan_date), '') IS NULL THEN 'Missing Loan Date'
        WHEN NULLIF(TRIM(due_date), '') IS NULL THEN 'Missing Due Date'
        WHEN due_date < loan_date THEN 'Invalid Due Date'
    END,
    NOW()
FROM bronze_loans
WHERE NULLIF(TRIM(loan_date), '') IS NULL
   OR NULLIF(TRIM(due_date), '') IS NULL
   OR (NULLIF(TRIM(loan_date), '') IS NOT NULL AND NULLIF(TRIM(due_date), '') IS NOT NULL AND due_date < loan_date);
...
WITH ranked_loans AS (
    SELECT *,
           NULLIF(TRIM(loan_date), '') AS loan_date_clean,
           NULLIF(TRIM(due_date), '') AS due_date_clean,
           NULLIF(TRIM(return_date), '') AS return_date_clean,
           ROW_NUMBER() OVER (PARTITION BY loan_id ORDER BY ingested_at DESC) AS rn
    FROM bronze_loans
)
 
SELECT loan_id,customer_id,book_id,loan_date_clean, due_date_clean,return_date_clean,
    GREATEST(
        DATEDIFF(
            COALESCE(return_date_clean, CURDATE()),
            due_date_clean
        ),
        0
    ) AS days_overdue,
    ...
    CASE
        WHEN GREATEST(DATEDIFF(COALESCE(return_date_clean, CURDATE()), due_date_clean),0) = 0
            THEN 'ON TIME'
 
        WHEN GREATEST(DATEDIFF(COALESCE(return_date_clean, CURDATE()), due_date_clean),0) BETWEEN 1 AND 5
            THEN 'MILD'
 
        WHEN GREATEST(DATEDIFF(COALESCE(return_date_clean, CURDATE()), due_date_clean),0) BETWEEN 6 AND 15
            THEN 'SEVERE'
...
WHERE rn = 1
AND loan_date_clean IS NOT NULL
AND due_date_clean IS NOT NULL
AND due_date_clean >= loan_date_clean
AND NOT EXISTS (SELECT 1 FROM silver_loans sl WHERE sl.loan_id = rl.loan_id);
...
WHERE rn = 1
  ...
  AND NOT EXISTS (SELECT 1 FROM silver_reviews sr WHERE sr.review_id = rr.review_id);